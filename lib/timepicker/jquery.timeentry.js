...
function TimeEntry() {
	...
	this._defaults = {
		...
		afterChange: null, // Function that triggered after changing the time
		allowRemove: true, // Parametre allow to use delete/backspace/ctrl+End buttons to remove value in field
	};
...
$.extend(TimeEntry.prototype, {
	...
	_doKeyDown: function(event) {
		...
		switch (event.keyCode) {
			case 8: if (!inst.options.allowRemove){ // preventing removing by baclspace if allowRemove option set to false
	...
	_setValue: function(inst, value) {
		if (value != inst.input.val()) {
			if(inst.options.allowRemove || value != '') {
				inst.input.val(value).trigger('change');

				setTimeout(function() { 
					$.extend(inst.options, ($.isFunction(inst.options.afterChange) ?
						inst.options.afterChange.apply(inst.input, [inst.input.val()]) : {}));
				}, 10);
			}
		}
	},
